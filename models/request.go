...
type RegisterRequest struct {
	Username string `json:"username" binding:"required,numeric,max=32"`      // 学号/工号，纯数字，≤32位
	Name     string `json:"name"     binding:"required,max=32"`              // 姓名，≤32位
	Password string `json:"password" binding:"required,min=8,max=16"`        // 密码，8~16位
